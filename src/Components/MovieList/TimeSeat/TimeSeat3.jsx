...
import { saveDataTimeSlice3 } from "../../../features/LoginPage/TimeSlice3.js";
import SeatTimeSlice3 from "../Seats/SeatTimeSlice3.jsx";

export default function TimeSeat3() {
  const { id, moviename, name, movieid } = useParams();
  let addSilver = useSelector((state) => state.TimeSlice3.Sliver);
  let addGold = useSelector((state) => state.TimeSlice3.Gold);
  let addPlatinum = useSelector((state) => state.TimeSlice3.Platinum);
  let TotalAmount = useSelector((state) => state.TimeSlice3.totalAmount);
